...
const filteredData = computed(() => {
  ...
  const adminArray = Object.values(data.value);

  return lowerCaseQuery
    ? adminArray.filter((adminManagemenet) =>
      adminManagemenet.fullname.toLowerCase().includes(lowerCaseQuery) ||
      adminManagemenet.email.toLowerCase().includes(lowerCaseQuery)
    )
    : adminArray;
});
